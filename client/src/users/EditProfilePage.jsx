...
export default function EditProfilePage() {
    ...
    if (isValid) {
      try {
        ...
        if (response.error) {
          ...
          setTimeout(() => {
            setFormData({ ...formData, redirectToProfile: true });
  ...
  if (formData.redirectToProfile) return <Navigate to={`/user/${userId}`} />;
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {status === STATUS.SUBMITTING ? "Submitting..." : "Update Profile"}
        </button>
      </form>
    </div>
  );
